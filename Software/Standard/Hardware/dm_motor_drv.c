...
void dm3519_current_set(uint16_t id, float m1_cur_set, float m2_cur_set, float m3_cur_set, float m4_cur_set)
{
	uint8_t data[8];
...
void dm_motor_ctrl_send(motor_t *motor)
{
	...
	{
		case mit_mode:
			mit_ctrl(motor, motor->id, motor->ctrl.pos_set, motor->ctrl.vel_set, motor->ctrl.kp_set, motor->ctrl.kd_set, motor->ctrl.tor_set);
			break;
		case pos_mode:
		...
		case spd_mode:
			...
			break;
		case psi_mode:
			psi_ctrl(motor->id,motor->ctrl.pos_set, motor->ctrl.vel_set, motor->ctrl.cur_set);
			break;
	}	
}
