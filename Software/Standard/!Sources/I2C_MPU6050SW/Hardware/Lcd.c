...
void LCD_Init(void)
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA|RCC_APB2Periph_GPIOD|RCC_APB2Periph_GPIOE,ENABLE); //使能PORTB,D,E以及AFIO复用功能时钟

		
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;           //PB8 推挽输出 背光
    ...
    GPIO_Init(GPIOE, &GPIO_InitStructure);
		...
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4 | GPIO_Pin_5 | GPIO_Pin_11; // PD4=RD, PD5=WR, PD11=RS(A16)
    GPIO_Init(GPIOD, &GPIO_InitStructure);
...
void LCD_ShowSignedNum(u16 x, u16 y, s32 num, u8 len, u8 size, u8 mode)
{
    u8 sign = 0; 
    u32 abs_num;
    ...
    LCD_ShowxNum(x, y, abs_num, len, size, mode);
}
...
void LCD_ShowHexNum(u16 x, u16 y, u32 num, u8 len, u8 size, u8 mode)
{
    u8 t;
    u8 hex_char;
    u8 fill_zero = (mode & 0x80) ? 1 : 0; 
    u8 nibble; 
    u8 enshow = 0; 
    ...
    for(t = 0; t < len; t++)
    {
        nibble = (num >> ((len - t - 1) * 4)) & 0x0F;        

        if(enshow || nibble || t == (len - 1))
        {
            enshow = 1; 
            if(nibble < 10) hex_char = nibble + '0';
            else hex_char = nibble - 10 + 'A';
            
            LCD_ShowChar(x + (size/2)*t, y, hex_char, size, mode & 0x01);
        }
        else if(fill_zero)
        {
...
void LCD_ShowBinNum(u16 x, u16 y, u32 num, u8 len, u8 size, u8 mode)
{
    u8 t;
    u8 bit;
    u8 fill_zero = (mode & 0x80) ? 1 : 0; 
    ...
    for(t = 0; t < len; t++)
    {
        bit = (num >> (len - t - 1)) & 0x01;

        if(enshow || bit || t == (len - 1))
        {
            enshow = 1; 
            LCD_ShowChar(x + (size/2)*t, y, bit + '0', size, mode & 0x01);
        }
        else if(fill_zero)
        {
            LCD_ShowChar(x + (size/2)*t, y, '0', size, mode & 0x01);
